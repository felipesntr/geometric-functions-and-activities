...
update (TimePassing t) ((x, y), (v_x, v_y), angle, rot, accelerating)
  ...
  | accelerating =
    if v_x <= velocityLimitX && v_y <= velocityLimitY
      then ((x + v_x * cos angle, y + v_y * sin angle), (v_x + t * accelerationShip, v_y + t * accelerationShip), angle, rot, accelerating)
      else ((x + v_x * cos angle, y + v_y * sin angle), (v_x, v_y), angle, rot, accelerating)
  | not accelerating && v_x >= 0 && v_y >= 0 = ((x + v_x * cos angle, y + v_y * sin angle), (v_x - t * accelerationShip, v_y - t * accelerationShip), angle, rot, accelerating)
